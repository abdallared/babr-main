...
export function Contact() {
  ...
  const field =
    'w-full border border-ink-line bg-foreground/[0.03] px-4 py-3.5 text-sm text-foreground placeholder:text-foreground/25 outline-none transition-colors duration-300 focus:border-babbr focus:bg-foreground/[0.05]'
                    ...
                    <a
                      ...
                      className={`clip-shear group flex items-center gap-4 border p-4 transition-all duration-400 ${
                        c.primary
                          ...
                          : 'border-ink-line bg-ink-soft hover:border-foreground/25'
                      }`}
                    >
                      <span
                        className={`grid size-11 shrink-0 place-items-center border ${
                          c.primary ? 'border-babbr/40 text-babbr' : 'border-ink-line text-foreground-muted'
                        }`}
                      >
                        <Icon className="size-[18px]" />
                      </span>
                      <div className="min-w-0 flex-1">
                        <p className="text-[11px] text-foreground/40">{c.label}</p>
                        <p dir="ltr" className="truncate text-end text-sm font-semibold text-foreground/90">
                          {c.value}
                        </p>
                      </div>
            ...
            >
              <div className="mb-7 flex items-center gap-3 border-b border-ink-line pb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-foreground-muted">دقيقة واحدة، وتوصلنا كل التفاصيل</p>
                </div>
              </div>

              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label htmlFor="c-name" className="mb-2 block text-xs text-foreground-muted">الاسم *</label>
                  <input
                ...
                <div>
                  <label htmlFor="c-brand" className="mb-2 block text-xs text-foreground-muted">اسم المشروع</label>
                  <input
                ...
                <div>
                  <label htmlFor="c-phone" className="mb-2 block text-xs text-foreground-muted">رقم الهاتف *</label>
                  <input
                ...
                <div>
                  <label htmlFor="c-budget" className="mb-2 block text-xs text-foreground-muted">الميزانية التقديرية</label>
                  <select
              ...
              <div className="mt-6">
                <p className="mb-3 text-xs text-foreground-muted">شنو المطلوب؟ (تقدر تختار أكثر من واحد)</p>
                      ...
                      <button
                        ...
                        className={`clip-shear flex items-center gap-1.5 border px-3.5 py-2 text-[13px] transition-all duration-300 ${
                          on
                            ...
                            : 'border-ink-line bg-foreground/[0.03] text-foreground-muted hover:border-foreground/25 hover:text-foreground/85'
                        }`}
                      >
              ...
              <div className="mt-6">
                <label htmlFor="c-note" className="mb-2 block text-xs text-foreground-muted">تفاصيل إضافية</label>
                <textarea
